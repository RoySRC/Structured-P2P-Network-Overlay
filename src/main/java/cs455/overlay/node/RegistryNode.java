...
public class RegistryNode implements Node {
     ...
     */
    public boolean flagOverlayNodesFinishedSetup = false;
     ...
     * to false;
     */
    public boolean flagSettingUpOverlay = false;

    /**
     * becomes true if the overlay nodes are currently routing messages, false otherwise
     */
    public boolean fOverlayNodesRoutingMessages = false;
        ...
        while (stdin.hasNext()) {
            ...
            switch (LineToken[0]) {
                case "list-messaging-nodes":
                    RegistryArgParser.list_messaging_nodes.action(this, LineToken);
                    break;

                case "setup-overlay":
                    RegistryArgParser.setup_overlay.action(this, LineToken);
                    break;

                case "list-routing-tables":
                    RegistryArgParser.list_routing_tables.action(this, LineToken);
                    break;

                case "set-wait-time":
                    RegistryArgParser.set_wait_time.action(this, LineToken);
                    break;

                case "start":
                    RegistryArgParser.start.action(this, LineToken);
                    break;
