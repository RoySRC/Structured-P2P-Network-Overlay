...
public class Peer implements Node {
     ...
     */
    public TCPConnectionsCache connectionsCache;

    /**
     * messaging node server for incoming messages
     */
    public TCPServerThread server = null;
     ...
     */
    public RoutingTable routing_table = null;

    /**
     * Connection to the Registry
     */
    public TCPConnection tcp_connection_registry = null;
        ...
        while (stdin.hasNext()) {
            switch (stdin.nextLine()) {
                case "print-counters-and-diagnostics":
                    PeerArgParser.print_counters_and_diagnostics.action(this);
                    break;

                case "exit-overlay":
                    log.info("Exit Overlay");
                    PeerArgParser.exit_overlay.action(this);
                    log.info("Sent OVERLAY_NODE_SENDS_DEREGISTRATION message to registry.");
                    break;

                case "print-routing-table":
                    PeerArgParser.print_routing_table.action(this);
                    break;
